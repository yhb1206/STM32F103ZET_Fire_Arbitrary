...
u08  gu08MnSymbleSize;
u08  gu08MbyModuleData[MAX_MODULESIZE][MAX_MODULESIZE];

int  m_ncDataCodeWordBit;
u08  m_byDataCodeWord[MAX_DATACODEWORD];

int  m_ncDataBlock;
u08  m_byBlockMode[MAX_DATACODEWORD];
u08  m_nBlockLength[MAX_DATACODEWORD];

int  m_ncAllCodeWord;
u08  m_byAllCodeWord[MAX_ALLCODEWORD];
u08  m_byRSWork[MAX_CODEBLOCK];
...
const QR_VERSIONINFO QrVersonInfo[] = 
{
    {0}, /* Ver.0*/
...
/*X^8+X^4+X^3+X^2+1*/
uc8	 byRSExp7[]  = {87, 229, 146, 149, 238, 102,  21};
uc8	 byRSExp10[] = {251,  67,  46,  61, 118,  70,  64,  94,  32,  45};
uc8	 byRSExp13[] = { 74, 152, 176, 100,  86, 100, 106, 104, 130, 218, 206, 140,  78};
uc8	 byRSExp15[] = 
{  
    8,   183,  61,  91, 202,  37,  51, 58,  
   58,   237, 140, 124,   5,  99, 105
};
uc8	 byRSExp16[] = 
{
    120, 104, 107, 109, 102, 161,  76,   3,  
...
uc8	 byRSExp18[] = 
{
    215, 234, 158,  94, 184,  97, 118, 170,  79, 
    187, 152, 148, 252, 179,   5,  98,  96, 153
};
uc8	 byRSExp20[] = 
{ 
    17,  60,  79,  50,  61, 163,  26, 187, 202, 180, 
   221, 225,  83, 239, 156, 164, 212, 212, 188, 190
};
uc8	 byRSExp22[] = 
{
    210, 171, 247, 242,  93, 230,  14, 109, 221,  53, 200,  
     74,   8, 172,  98,  80, 219, 134, 160, 105, 165, 231
};
uc8	 byRSExp24[] = 
{
    229, 121, 135,  48, 211, 117, 251, 126, 159, 180, 169, 152, 
    192, 226, 228, 218, 111,   0, 117, 232,  87,  96, 227,  21
};
uc8	 byRSExp26[] = 
{
    173, 125, 158,   2, 103, 182, 118,  17, 145, 201, 111,  28, 165,  
     53, 161,  21, 245, 142,  13, 102,  48, 227, 153, 145, 218,  70
};
uc8	 byRSExp28[] = 
{
    168, 223, 200, 104, 224, 234, 108, 180, 110, 190, 195, 147, 205,  27, 
...
uc8	 byRSExp38[] = 
{
    159,  34,  38, 228, 230,  59, 243,  95,  49, 218, 176, 164,  20,  65,  45, 111,  
...
uc8	 byRSExp54[] = 
{
    183,  26, 201,  87, 210, 221, 113,  21,  46,  65,  45,  50, 238, 184, 249, 225, 
...
uc8	 nIndicatorLenNumeral[]  =   {10, 12, 14};
uc8	 nIndicatorLenAlphabet[] =   { 9, 11, 13};
uc8	 nIndicatorLen8Bit[]	   = { 8, 16, 16};
uc8	 nIndicatorLenKanji[]	   = { 8, 10, 12};
...
;***************************************************************************
*/
static u08 GetEncodeVersion(int nVersion, uc8 *lpsSource, int ncLength);
static int EncodeSourceData(uc8 *lpsSource, int ncLength, int nVerGroup);
static int GetBitLength(u08  nMode, int ncData, int nVerGroup);
static int SetBitStream(int nIndex, u16 wData, int ncData);
static u08 IsNumeralData(u08 c);
static u08 IsAlphabetData(u08 c);
static u08 IsKanjiData(u08 c1, u08 c2);
static u08 IsChineseData(u08 c1, u08 c2);
static u08 AlphabetToBinaly(u08 c);
static u16 KanjiToBinaly(u16 wc);
static u16 ChineseToBinaly(u16 wc);
static void GetRSCodeWord(u08 *lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord);
...
/*************************************************
 * FUNCTION:EncodeData
 * DESCRIPTION:QR二维码生成原始点数函数
 * INPUT:lpsSource 源数据, u16len长度 ,
 * OUTPUT:
 * RETURN: GAL_YES:生成成功 FALSE生成失败
 * OTHERS:
 * HISTORY:
*************************************************/
u08 EncodeData(uc8 *lpsSource, u16 u16len)
{
	int i, j, nVersion=1, bAutoExtent=1;
	int ncDataCodeWord, ncTerminater;
    u08 nEncodeVersion;
	u08	byPaddingCode = 0xec;
	int nDataCwIndex = 0, ncBlock1, ncBlock2, ncBlockSum;
	int nBlockNo = 0, ncDataCw1, ncDataCw2;
	int ncRSCw1, ncRSCw2;

	m_nLevel = QR_LEVEL;/*纠错等级，0=L,1=M,2=Q,3=H*/
	m_nMaskingNo = 0;

    /*《QR国家标准》P12:如果用户没有指定所采用的符号版本，则选择与数据相适应的最小的版本。表1为全部符号版本及其容量。*/
	 nEncodeVersion = GetEncodeVersion(nVersion, lpsSource, u16len);/*计算编码数据需要的版本*/

	if((nEncodeVersion == 0) || (nEncodeVersion > QR_VERSION))
	{
        return GAL_NO;
	}
	...
	ncDataCodeWord = QrVersonInfo[m_nVersion].ncDataCodeWord[m_nLevel];
	...
	for (i = (m_ncDataCodeWordBit + 7) / 8; i < ncDataCodeWord; ++i)
	{
		...
		byPaddingCode = (u08)(byPaddingCode == 0xec ? 0x11 : 0xec);
	}

	m_ncAllCodeWord = QrVersonInfo[m_nVersion].ncAllCodeWord;
	memset(m_byAllCodeWord, 0, m_ncAllCodeWord);

	ncBlock1 = QrVersonInfo[m_nVersion].RS_BlockInfo1[m_nLevel].ncRSBlock;
	ncBlock2 = QrVersonInfo[m_nVersion].RS_BlockInfo2[m_nLevel].ncRSBlock;
	ncBlockSum = ncBlock1 + ncBlock2;

	ncDataCw1 = QrVersonInfo[m_nVersion].RS_BlockInfo1[m_nLevel].ncDataCodeWord;
	ncDataCw2 = QrVersonInfo[m_nVersion].RS_BlockInfo2[m_nLevel].ncDataCodeWord;
	...
	ncRSCw1 = QrVersonInfo[m_nVersion].RS_BlockInfo1[m_nLevel].ncAllCodeWord - ncDataCw1;
	ncRSCw2 = QrVersonInfo[m_nVersion].RS_BlockInfo2[m_nLevel].ncAllCodeWord - ncDataCw2;
	...
	gu08MnSymbleSize = m_nVersion * 4 + 17;

	FormatModule();

	return QrSideCal(nEncodeVersion);
}

/*************************************************
 * FUNCTION:GetEncodeVersion
 * DESCRIPTION:计算最小版本
 * INPUT:nVersion 预估版本, lpsSource数据源 ncLength 数据长度,
 * OUTPUT:
 * RETURN:  0版本计算错误     1-40计算的版本
 * OTHERS:
 *  
 *                   版本        每个版本长度字段的位数        
...
*************************************************/
u08 GetEncodeVersion(int nVersion, uc8 *lpsSource, int ncLength)
{
    /*如果知道版本是多少就少浪费点时间寻找最优的版本了，否则的话就从版本1移植寻找到版本40，这是针对国家标准没有指定版本的情况下的处理*/
	...
	for (i = nVerGroup; i <= QR_VRESION_L; ++i)
	{
		if (EncodeSourceData(lpsSource, ncLength, i))
		{
			if (i == QR_VRESION_S)
			{
				for (j = 1; j <= 9; ++j)
				{
					if ((m_ncDataCodeWordBit + 7) / 8 <= QrVersonInfo[j].ncDataCodeWord[m_nLevel])
					{
                        return j;
...
/*ncLength	数据长度*/
/*nVerGroup	版本组*/
/*u08 EncodeSourceData(u08 *lpsSource, int ncLength, int nVerGroup)*/
int EncodeSourceData(uc8 *lpsSource, int ncLength, int nVerGroup)
{
	int i, j;
	...
	for (i = 0; i < m_ncDataBlock && m_ncDataCodeWordBit != -1; ++i)
	{
		...
		if (m_byBlockMode[i] == QR_MODE_NUMERAL)
		{
			...
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (u16)m_nBlockLength[i], nIndicatorLenNumeral[nVerGroup]);

			for (j = 0; j < m_nBlockLength[i]; j += 3)
			{
				if (j < m_nBlockLength[i] - 2)
				{
					wBinCode = (u16)(((lpsSource[ncComplete + j]	  - '0') * 100) +
				...
				{
					wBinCode = (u16)(((lpsSource[ncComplete + j] - '0') * 10) +
				...
				{
					wBinCode = (u16)(lpsSource[ncComplete + j] - '0');
		...
		else if (m_byBlockMode[i] == QR_MODE_ALPHABET)/*如果数据块是字符模式*/
		{
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 2, 4);

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (u16)m_nBlockLength[i], nIndicatorLenAlphabet[nVerGroup]);

			for (j = 0; j < m_nBlockLength[i]; j += 2)
			{
				if (j < m_nBlockLength[i] - 1)
				{
					wBinCode = (u16)((AlphabetToBinaly(lpsSource[ncComplete + j]) * 45) +
				...
				{
					wBinCode = (u16)AlphabetToBinaly(lpsSource[ncComplete + j]);
		...
		else if (m_byBlockMode[i] == QR_MODE_8BIT) /*如果数据块是8位字节模式*/
		{
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, 4, 4);

			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (u16)m_nBlockLength[i], nIndicatorLen8Bit[nVerGroup]);

			for (j = 0; j < m_nBlockLength[i]; ++j)
			{
				m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (u16)lpsSource[ncComplete + j], 8);
			}

			ncComplete += m_nBlockLength[i];
		}
        #if(CHINESE_SYMBOL == GAL_YES)
        else if(m_byBlockMode[i] == QR_MODE_KANJI) /*如果数据块是日文模式*/
        #else
		...
		{/*日本模式*/
			...
			m_ncDataCodeWordBit = SetBitStream(m_ncDataCodeWordBit, (u16)(m_nBlockLength[i] / 2), nIndicatorLenKanji[nVerGroup]);

			for (j = 0; j < m_nBlockLength[i] / 2; ++j)
			{
				u16 wBinCode = KanjiToBinaly((u16)(((u08)lpsSource[ncComplete + (j * 2)] << 8) + (u08)lpsSource[ncComplete + (j * 2) + 1]));
...
/*获取bit长度*/
int GetBitLength(u08 nMode, int ncData, int nVerGroup)
{
	int ncBits = 0;
...
/*设置bit流*/
int SetBitStream(int nIndex, u16 wData, int ncData)
{
	int i;
...
u08 IsChineseData(u08 c1, u08 c2)
{
	/*中文模式*/
	if((c1 >= 0xa1 && c1 < 0xaa) || (c1 >= 0xb0 && c1 <= 0xfa))
	{
		if(c2 >= 0xa1 && c2 <= 0xfe)	return GAL_YES;
	}
	return GAL_NO;
}

/*字母表信息转化*/
u08 AlphabetToBinaly(u08 c)
{
	if (c >= '0' && c <= '9') return (u08)(c - '0');

	if (c >= 'A' && c <= 'Z') return (u08)(c - 'A' + 10);
...
u16 KanjiToBinaly(u16 wc)
{
	...
	return (u16)(((wc >> 8) * 0xc0) + (wc & 0x00ff));
}
...
e)将结果转换为13位二进制串
*/
u16 ChineseToBinaly(u16 wc)
{
	if(wc >= 0xa1a1 && wc <= 0xa9fe)
	{
		return (u16)((((wc >> 8)-0xa1) * 0x60) + ((wc & 0x00ff)-0xa1));
	}
	if(wc >= 0xb0a1 && wc <= 0xf9fe)
	{
		return (u16)((((wc >> 8)-0xa6) * 0x60) + ((wc & 0x00ff)-0xa1));
	}
	return (u16)((((wc >> 8)-0xa6) * 0x60) + ((wc & 0x00ff)-0xa1));
}

/*纠错码,Reed-Solomon error correction（里德-所罗门纠错算法）多项式除法，把1-255的数映射成2的n次方（0<=n<=255）的伽罗瓦域Galois Field*/
void GetRSCodeWord(u08 *lpbyRSWork, int ncDataCodeWord, int ncRSCodeWord)
{
	int i, j;
	...
	for (i = 0; i < ncDataCodeWord ; ++i)
	{
		if (lpbyRSWork[0] != 0)
		{
			u08 nExpFirst = byIntToExp[lpbyRSWork[0]];

			for (j = 0; j < ncRSCodeWord; ++j)
			{
				u08 nExpElement = (u08)(((int)(byRSExp[ncRSCodeWord][j] + nExpFirst)) % 255);

				lpbyRSWork[j] = (u08)(lpbyRSWork[j + 1] ^ byExpToInt[nExpElement]);
			}
...
void FormatModule(void)
{
	...
	memset(gu08MbyModuleData, 0, sizeof(gu08MbyModuleData));
	...
	for (i = 0; i < gu08MnSymbleSize; ++i)
	{
		for (j = 0; j < gu08MnSymbleSize; ++j)
		{
			gu08MbyModuleData[i][j] = (u08)((gu08MbyModuleData[i][j] & 0x11) != 0);
		}
	}
}
...
void SetFunctionModule(void)
{
	...
	SetFinderPattern(gu08MnSymbleSize - 7, 0);
	SetFinderPattern(0, gu08MnSymbleSize - 7);

	for (i = 0; i < 8; ++i)
	{
		gu08MbyModuleData[i][7] = gu08MbyModuleData[7][i] = '\x20';
		gu08MbyModuleData[gu08MnSymbleSize - 8][i] = gu08MbyModuleData[gu08MnSymbleSize - 8 + i][7] = '\x20';
		gu08MbyModuleData[i][gu08MnSymbleSize - 8] = gu08MbyModuleData[7][gu08MnSymbleSize - 8 + i] = '\x20';
	}

	for (i = 0; i < 9; ++i)
	{
		gu08MbyModuleData[i][8] = gu08MbyModuleData[8][i] = '\x20';
	}

	for (i = 0; i < 8; ++i)
	{
		gu08MbyModuleData[gu08MnSymbleSize - 8 + i][8] = gu08MbyModuleData[8][gu08MnSymbleSize - 8 + i] = '\x20';
	}

	SetVersionPattern();	/*版本信息*/

	for (i = 0; i < QrVersonInfo[m_nVersion].ncAlignPoint; ++i)
	{
		SetAlignmentPattern(QrVersonInfo[m_nVersion].nAlignPoint[i], 6);/*小回字*/
		SetAlignmentPattern(6, QrVersonInfo[m_nVersion].nAlignPoint[i]);

		for (j = 0; j < QrVersonInfo[m_nVersion].ncAlignPoint; ++j)
		{
			SetAlignmentPattern(QrVersonInfo[m_nVersion].nAlignPoint[i], QrVersonInfo[m_nVersion].nAlignPoint[j]);
		}
	}

	for (i = 8; i <= gu08MnSymbleSize - 9; ++i)
	{
		gu08MbyModuleData[i][6] = (i % 2) == 0 ? '\x30' : '\x20';
		gu08MbyModuleData[6][i] = (i % 2) == 0 ? '\x30' : '\x20';
	}
}
...
void SetFinderPattern(int x, int y)
{
	...
	for (i = 0; i < 7; ++i)
	{
		for (j = 0; j < 7; ++j)
		{
			gu08MbyModuleData[x + j][y + i] = (byPattern[i] & (1 << (6 - j))) ? '\x30' : '\x20';
		}
	}
}
...
void SetAlignmentPattern(int x, int y)
{
	...
	for (i = 0; i < 5; ++i)
	{
		for (j = 0; j < 5; ++j)
		{
			gu08MbyModuleData[x + j][y + i] = (byPattern[i] & (1 << (4 - j))) ? '\x30' : '\x20';
		}
	}
}
...
void SetVersionPattern(void)
{
	...
	for (i = 0; i < 6; ++i)
	{
		for (j = 0; j < 3; ++j)
		{
			gu08MbyModuleData[gu08MnSymbleSize - 11 + j][i] = gu08MbyModuleData[i][gu08MnSymbleSize - 11 + j] =
...
{
	int x = gu08MnSymbleSize;
	int y = gu08MnSymbleSize - 1;
		...
		for (j = 0; j < 8; ++j)
		{
			do
			{
				...
				if (nCoef_x < 0)
				{
					...
					if (y < 0 || y == gu08MnSymbleSize)
					{
						y = (y < 0) ? 0 : gu08MnSymbleSize - 1;
						nCoef_y *= -1;
			...
			while (gu08MbyModuleData[x][y] & 0x20);

			gu08MbyModuleData[x][y] = (m_byAllCodeWord[i] & (1 << (7 - j))) ? '\x02' : '\x00';
		}
	}
}
...
void SetMaskingPattern(int nPatternNo)
{
	...
	for (i = 0; i < gu08MnSymbleSize; ++i)
	{
		for (j = 0; j < gu08MnSymbleSize; ++j)
		{
			if (! (gu08MbyModuleData[j][i] & 0x20))
			{
				switch (nPatternNo)
				{
				case 0:
					bMask = ((i + j) % 2 == 0)?GAL_YES:GAL_NO;
					break;

				case 1:
					bMask = (i % 2 == 0)?GAL_YES:GAL_NO;
					break;

				case 2:
					bMask = (j % 3 == 0)?GAL_YES:GAL_NO;
					break;

				case 3:
					bMask = ((i + j) % 3 == 0)?GAL_YES:GAL_NO;
					break;

				case 4:
					bMask = (((i / 2) + (j / 3)) % 2 == 0)?GAL_YES:GAL_NO;
					break;

				case 5:
					bMask = (((i * j) % 2) + ((i * j) % 3) == 0)?GAL_YES:GAL_NO;
					break;

				case 6:
					bMask = ((((i * j) % 2) + ((i * j) % 3)) % 2 == 0)?GAL_YES:GAL_NO;
					break;

				default:
					bMask = ((((i * j) % 3) + ((i + j) % 2)) % 2 == 0)?GAL_YES:GAL_NO;
					break;
				}

				gu08MbyModuleData[j][i] = (u08)((gu08MbyModuleData[j][i] & 0xfe) | (((gu08MbyModuleData[j][i] & 0x02) > 1) ^ bMask));
			}
		}
	}
}
...
void SetFormatInfoPattern(int nPatternNo)
{
	...
	for (i = 0; i <= 5; ++i)
		gu08MbyModuleData[8][i] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	gu08MbyModuleData[8][7] = (nFormatData & (1 << 6)) ? '\x30' : '\x20';
	gu08MbyModuleData[8][8] = (nFormatData & (1 << 7)) ? '\x30' : '\x20';
	gu08MbyModuleData[7][8] = (nFormatData & (1 << 8)) ? '\x30' : '\x20';

	for (i = 9; i <= 14; ++i)
		gu08MbyModuleData[14 - i][8] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	for (i = 0; i <= 7; ++i)
		gu08MbyModuleData[gu08MnSymbleSize - 1 - i][8] = (nFormatData & (1 << i)) ? '\x30' : '\x20';

	gu08MbyModuleData[8][gu08MnSymbleSize - 8] = '\x30';

	for (i = 8; i <= 14; ++i)
		gu08MbyModuleData[8][gu08MnSymbleSize - 15 + i] = (nFormatData & (1 << i)) ? '\x30' : '\x20';
}
...
int CountPenalty(void)
{
	...
	for (i = 0; i < gu08MnSymbleSize; ++i)
	{
		for (j = 0; j < gu08MnSymbleSize - 4; ++j)
		{
			int nCount = 1;

			for (k = j + 1; k < gu08MnSymbleSize; k++)
			{
				if (((gu08MbyModuleData[i][j] & 0x11) == 0) == ((gu08MbyModuleData[i][k] & 0x11) == 0))
					++nCount;
				else
					break;
			}
	...
	for (i = 0; i < gu08MnSymbleSize; ++i)
	{
		for (j = 0; j < gu08MnSymbleSize - 4; ++j)
		{
			...
			for (k = j + 1; k < gu08MnSymbleSize; k++)
			{
				if (((gu08MbyModuleData[j][i] & 0x11) == 0) == ((gu08MbyModuleData[k][i] & 0x11) == 0))
					++nCount;
				else
					break;
			}
	...
	for (i = 0; i < gu08MnSymbleSize - 1; ++i)
	{
		for (j = 0; j < gu08MnSymbleSize - 1; ++j)
		{
			if ((((gu08MbyModuleData[i][j] & 0x11) == 0) == ((gu08MbyModuleData[i + 1][j]		& 0x11) == 0)) &&
				(((gu08MbyModuleData[i][j] & 0x11) == 0) == ((gu08MbyModuleData[i]	[j + 1] & 0x11) == 0)) &&
				(((gu08MbyModuleData[i][j] & 0x11) == 0) == ((gu08MbyModuleData[i + 1][j + 1] & 0x11) == 0)))
			{
				nPenalty += 3;
			}
		}
	...
	for (i = 0; i < gu08MnSymbleSize; ++i)
	{
		for (j = 0; j < gu08MnSymbleSize - 6; ++j)
		{
			if (((j == 0) ||				 (! (gu08MbyModuleData[i][j - 1] & 0x11))) && /* 柧 傑偨偼 僔儞儃儖奜*/
											 (   gu08MbyModuleData[i][j]     & 0x11)   && /* 埫 - 1*/
											 (! (gu08MbyModuleData[i][j + 1] & 0x11))  && /* 柧 - 1*/
											 (   gu08MbyModuleData[i][j + 2] & 0x11)   && /* 埫 劉*/
											 (   gu08MbyModuleData[i][j + 3] & 0x11)   && /* 埫 劆3*/
											 (   gu08MbyModuleData[i][j + 4] & 0x11)   && /* 埫 劊*/
											 (! (gu08MbyModuleData[i][j + 5] & 0x11))  && /* 柧 - 1*/
											 (   gu08MbyModuleData[i][j + 6] & 0x11)   && /* 埫 - 1*/
				((j == gu08MnSymbleSize - 7) || (! (gu08MbyModuleData[i][j + 7] & 0x11))))   /* 柧 傑偨偼 僔儞儃儖奜*/
			{
				if (((j < 2 || ! (gu08MbyModuleData[i][j - 2] & 0x11)) &&
					 (j < 3 || ! (gu08MbyModuleData[i][j - 3] & 0x11)) &&
					 (j < 4 || ! (gu08MbyModuleData[i][j - 4] & 0x11))) ||
					((j >= gu08MnSymbleSize - 8  || ! (gu08MbyModuleData[i][j + 8]  & 0x11)) &&
					 (j >= gu08MnSymbleSize - 9  || ! (gu08MbyModuleData[i][j + 9]  & 0x11)) &&
					 (j >= gu08MnSymbleSize - 10 || ! (gu08MbyModuleData[i][j + 10] & 0x11))))
				{
					nPenalty += 40;
				}
			}
	...
	for (i = 0; i < gu08MnSymbleSize; ++i)
	{
		for (j = 0; j < gu08MnSymbleSize - 6; ++j)
		{
			if (((j == 0) ||				 (! (gu08MbyModuleData[j - 1][i] & 0x11))) && /* 柧 傑偨偼 僔儞儃儖奜*/
											 (   gu08MbyModuleData[j]    [i] & 0x11)   && /* 埫 - 1*/
											 (! (gu08MbyModuleData[j + 1][i] & 0x11))  && /* 柧 - 1*/
											 (   gu08MbyModuleData[j + 2][i] & 0x11)   && /* 埫 劉*/
											 (   gu08MbyModuleData[j + 3][i] & 0x11)   && /* 埫 劆3*/
											 (   gu08MbyModuleData[j + 4][i] & 0x11)   && /* 埫 劊*/
											 (! (gu08MbyModuleData[j + 5][i] & 0x11))  && /* 柧 - 1*/
											 (   gu08MbyModuleData[j + 6][i] & 0x11)   && /* 埫 - 1*/
				((j == gu08MnSymbleSize - 7) || (! (gu08MbyModuleData[j + 7][i] & 0x11))))   /* 柧 傑偨偼 僔儞儃儖奜*/
			{
				if (((j < 2 || ! (gu08MbyModuleData[j - 2][i] & 0x11)) &&
					 (j < 3 || ! (gu08MbyModuleData[j - 3][i] & 0x11)) &&
					 (j < 4 || ! (gu08MbyModuleData[j - 4][i] & 0x11))) ||
					((j >= gu08MnSymbleSize - 8  || ! (gu08MbyModuleData[j + 8][i]  & 0x11)) &&
					 (j >= gu08MnSymbleSize - 9  || ! (gu08MbyModuleData[j + 9][i]  & 0x11)) &&
					 (j >= gu08MnSymbleSize - 10 || ! (gu08MbyModuleData[j + 10][i] & 0x11))))
				{
					nPenalty += 40;
				}
			}
	...
	if( (50 - ((nCount * 100) / (gu08MnSymbleSize * gu08MnSymbleSize))) > 0)
		s_nCount = 50 - ((nCount * 100) / (gu08MnSymbleSize * gu08MnSymbleSize));
	else
		s_nCount = 0 - (50 - ((nCount * 100) / (gu08MnSymbleSize * gu08MnSymbleSize)));
