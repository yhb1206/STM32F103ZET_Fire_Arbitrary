...
#define LF_BUF_MAX			(250) // (510)
#define LF_PATHSIZE			LF_BUF_MAX
#undef  VALUE_MAX_STRING
#define VALUE_MAX_STRING	(256 - 48)
#else
...
#else
typedef struct dfs_stat gstat_t;
#endif
...
#define B_DEFAULT_MEM	(16 * 1024)			/* Default memory allocation */
...
#define B_MAX_BLOCKS	(32 * 1024)			/* Maximum allocated blocks */
#else
#define B_SHIFT			4					/* Convert size to class */
#define B_ROUND			((1 << (B_SHIFT)) - 1)
#define B_MAX_CLASS		13					/* Maximum class number + 1 */
...
#ifdef LITTLEFOOT
#define SOCKET_BUFSIZ			250 	/* Underlying buffer size (old 510) */
#else
...
#ifdef NO_BALLOC
#ifdef RTT
	#define balloc(B_ARGS, num) rt_malloc(num)
	#define bfree(B_ARGS, p) rt_free(p)
	#define bfreeSafe(B_ARGS, p) \
		if (p != RT_NULL) { rt_free(p); } else
	#define brealloc(B_ARGS, p, num) rt_realloc(p, num)
#else
